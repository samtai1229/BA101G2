...
public class RentOrdServlet extends HttpServlet {
	...
	if ("redirect_to_login".equals(action)){
		String url ="/backend/member/member.do";
...
//			+ "  ?, ?, ?)";			
			...
			//3.
			String rentno = roSvc.findRentnoByMemnoAndStartdate(memno, startdate);
...
//quick_search_product_3	
	if ("quick_search_product_3".equals(action)){ 
		...
		try {
			...
			templist = efro.getLeasableEquipsByEcno("EC01");
			for(EquipmentVO eVO: templist){
				if(ecno1_List.size()<ecno1){
					ecno1_List.add(eVO);
					eVOList.add(eVO);
					str.append(eVO.getEmtno()+" ");
				}
			}
			
			templist = efro.getLeasableEquipsByEcno("EC02");
			for(EquipmentVO eVO: templist){
			...
			templist = efro.getLeasableEquipsByEcno("EC03");
			for(EquipmentVO eVO: templist){
			...
			templist = efro.getLeasableEquipsByEcno("EC04");
			for(EquipmentVO eVO: templist){
...
//quick_search_product_2	
	if ("quick_search_product_2".equals(action)){ 
		...
		try {
			...
			//3.在DB裝備庫存內，從每種裝備調出兩筆可租用(leasable)裝備
